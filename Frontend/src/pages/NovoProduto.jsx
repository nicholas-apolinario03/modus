...
export default function NovoProduto() {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                setValoresAtributos(prev => {
                    const novos = { ...prev };
                    data.forEach(attr => {
                        if (novos[attr.id] === undefined) novos[attr.id] = "";
                    });
                    return novos;
                });
            }
    ...
    const carregarDadosParaEdicao = async () => {
        try {
            ...
            if (data) {
                ...
                // 2. Mapeia os atributos atuais
                const atuais = {};
                data.attributes?.forEach(attr => {
                    atuais[attr.id] = attr.value_name;
                });
                setValoresAtributos(atuais);
                
                // 3. AGORA sim busca os requisitos da categoria carregada
                await buscarRequisitos(data.category_id);
            }
        } catch (err) {
    ...
    // --- RESTANTE DAS FUNÇÕES (Submit, Estilos, etc.) ---
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(endpoint, {
                ...
                body: JSON.stringify({ ...produto, atributos: atributosFormatados, usuarioId: user.id })
            });
    ...
    return (
        <div style={{ color: 'white', padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        {atributosRequeridos.filter(a => a.ehObrigatorio).map(attr => (
                            <div key={attr.id} style={{ marginBottom: '15px' }}>
                                ...
                                {attr.values?.length > 0 ? (
                                    ...
                                    <input type="text" required style={estiloInput(true)} value={valoresAtributos[attr.id] || ""} onChange={e => setValoresAtributos({ ...valoresAtributos, [attr.id]: e.target.value })} />
                                )}
                            </div>
                        ))}
                        {/* Botão de opcionais aqui... */}
                    </div>
                )}
                ...
                <button type="submit" style={{ padding: '15px', backgroundColor: '#3483fa', color: 'white', borderRadius: '4px', cursor: 'pointer' }}>
